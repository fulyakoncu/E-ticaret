    ...
    public partial class SiteMaster : System.Web.UI.MasterPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if ((!Page.IsPostBack))
                {
                    ...
                    if (UIAraclari._dtSepet.Columns.Count == 0)
        ...
        private void UyePaneli()
        {
            try
            {
                ...
                if ((eKullaniciTipi)UIAraclari._iKullaniciTipi == eKullaniciTipi.Yonetici)
                {
                    //Genel Tanımlar
        ...
        private void OlusturKullaniciSepeti()
        {
            try
            {
                UIAraclari._dtSepet.Columns.Add("SIRANO", typeof(int));
                UIAraclari._dtSepet.Columns.Add("URUNID", typeof(int));
                UIAraclari._dtSepet.Columns.Add("MIKTAR", typeof(int));
                UIAraclari._dtSepet.Columns.Add("URUNADI", typeof(string));
                UIAraclari._dtSepet.Columns.Add("MARKAADI", typeof(string));
            }
            catch (Exception ex)
            {
                UIAraclari.cLog.Write(ex, UIAraclari._iKullaniciID);
        ...
        protected void gvSepet_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                UIAraclari._dtSepet.Rows.Remove(UIAraclari._dtSepet.Select("SIRANO=" + gvSepet.SelectedDataKey["SIRANO"].ToString())[0]);
                DoldurSepet();
            }
            catch (Exception ex)
            {
