    ...
    public partial class UrunDetay : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                    ...
                    using (cUrunGenel UI = new cUrunGenel())
                    {
                        ...
                        if ((eParaBirimi)dr["PARABIRIMI"].ToShort() != eParaBirimi.TL)
                        {
                            ...
                            if (dtUrunGruplari.Select("GRUP_TIPI=" + (short)eGrupTipi.SabitKurDolar).Length > 0)
                                ...
                                ltTLDegeri.Text = (((eEvetHayir)dr["INDIRIM"].ToShort(0) == eEvetHayir.Evet ? dr["INDIRIMLI_FIYAT"].ToDecimal() : dr["FIYATI"].ToDecimal()) * UIAraclari.KurDegeri((eParaBirimi)dr["PARABIRIMI"].ToShort())).ToString();
                        }
                ...
                if (UIAraclari._iKullaniciID == 0)
                {
                    ...
                    //UIAraclari.toastMesaj(this, eStatusType.Hata, "Lütfen Üye Girşi Yapınız");
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnKaydet_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                yorumlar.UYE_ID = UIAraclari._iKullaniciID;
